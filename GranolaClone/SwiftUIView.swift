...
struct SwiftUIView: View {
    ...
    func transcribeCurrentFile(path: String) async throws {
        guard let audioFileBuffer = try? AudioProcessor.loadAudio(fromPath: path) else {
            return
        }
    ...
    func transcribeAudioSamples(_ samples: [Float]) async throws -> TranscriptionResult? {
        ...
        let transcription : TranscriptionResult? = try await whisperKit.transcribe(audioArray: samples, decodeOptions: options, callback: decodingCallback)
